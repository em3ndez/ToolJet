...
describe('folders controller', () => {
  ...
  it('should scope folders if user dont have folder create permission', async () => {
    const adminUserData = await createUser(nestApp, {
      email: 'admin@tooljet.io',
    });
    ...
    // new user can only see any folders without having apps with access
    response = await request(nestApp.getHttpServer())
      .get(`/api/folders`)
    ...
    // new user can only see any folders having apps with access
    ...
    // new user can only see all folders with folder create but apps are scoped
